...
import FileSelect, { globalFileData, resetAllGlobalData } from '~/app/FileSelect';
...
export default function CreateSecret({ onBack, showContent = true }: CreateSecretProps) {
    const { isDarkColorScheme } = useColorScheme();
    const [step, setStep] = React.useState(0);
    const [canProceed, setCanProceed] = React.useState(false);
    const [canProceedOptions, setCanProceedOptions] = React.useState(false);
    const leftOpacity = useSharedValue(1); // Always visible now
    const leftColor = useSharedValue("#dc2626"); // Start with red for cancel
    ...
    // Add left button border for red styling
    const leftBorder = useSharedValue("#b91c1c");
    ...
    React.useEffect(() => {
        ...
        // Update left button appearance based on step
        if (step === 0) {
            // Step 0: Red cancel button
            leftColor.value = withTiming("#dc2626", { duration: 300 });
            leftBorder.value = withTiming("#b91c1c", { duration: 300 });
        } else {
            // Step 1+: Normal back button
            leftColor.value = withTiming(isDarkColorScheme ? "#1f2937" : "#4b5563", { duration: 300 });
            leftBorder.value = withTiming(isDarkColorScheme ? "#4b5563" : "#d1d5db", { duration: 300 });
        }
    ...
    function decrement() {
        if (step > 0) {
            ...
            setTimeout(() => {
                ...
                // Left button appearance is now handled by the step useEffect
            ...
            // If we're at the first step, reset all data and go back to landing
            resetAllGlobalData();
            onBack();
        }
    }
                            ...
                            <>
                                <Animated.View className="rounded-full"
                                    ...
                                    <Pressable className="justify-center items-center"
                                        ...
                                        <Ionicon name={step === 0 ? "close" : "arrow-back"}
                                            size={24}
                                            color="white"/>
                                    </Pressable>
                                </Animated.View>
